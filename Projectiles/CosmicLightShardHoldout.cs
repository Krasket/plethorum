using Plethorum.Items.Weapons.Magic;
...
{
	public class CosmicLightShardHoldout : ModProjectile
	{
		...
		private const float MaxManaConsumptionDelay = 30f;
		private const float MinManaConsumptionDelay = 20f;
		...
		{
			DisplayName.SetDefault("Cosmic Light Shard");
		...
		private void FireBeams()
		{
			...
			for (int b = 0; b < NumBeams; ++b) {
				Projectile.NewProjectile(projectile.Center, beamVelocity, ModContent.ProjectileType<CosmicLightShardBeam>(), damage, knockback, projectile.owner, b, uuid);
			}
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
		{
			...
			Color drawColor = CosmicLightShard.OverrideColor;
