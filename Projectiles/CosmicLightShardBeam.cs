...
{
	public class CosmicLightShardBeam : ModProjectile
	{
		
		private const float PiBeamDivisor = MathHelper.Pi / CosmicLightShardHoldout.NumBeams;
		
		private const float MaxDamageMultiplier = 1.5f;
		
		private const float MaxBeamScale = 1.5f;
		...
		private const float VisualEffectThreshold = 1f;
		private const float OuterBeamOpacityMultiplier = 0.5f;
		private const float InnerBeamOpacityMultiplier = 0.1f;
		private const float BeamLightBrightness = 20f;
		
		private const float BeamColorHue = 0.75f;
		private const float BeamHueVariance = 0.2f;
		private const float BeamColorSaturation = 0.45f;
		private const float BeamColorLightness = 0.8f;
		...
		{
			DisplayName.SetDefault("Cosmic Light Shard Beam");
		}
		...
		public override void AI()
		{
			...
			if (projectile.type != ModContent.ProjectileType<CosmicLightShardBeam>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<CosmicLightShardHoldout>())
			{
				projectile.Kill();
				return;
			}
			...
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / CosmicLightShardHoldout.MaxCharge, 0f, 1f);

			projectile.damage = (int)(hostPrism.damage * GetDamageMultiplier(chargeRatio));

			projectile.friendly = hostPrism.ai[0] > CosmicLightShardHoldout.DamageStart;

			float beamIdOffset = BeamID - CosmicLightShardHoldout.NumBeams / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * CosmicLightShardHoldout.NumBeams) * MathHelper.TwoPi;
		...
		private Color GetOuterBeamColor()
		{
			...
			float hue = (BeamID / CosmicLightShardHoldout.NumBeams) % BeamHueVariance + BeamColorHue;
